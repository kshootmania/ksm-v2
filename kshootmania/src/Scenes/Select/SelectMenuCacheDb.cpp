...
Optional<SelectCachedChart> SelectMenuCacheDb::FindChart(FilePathView chartPath, bool forceRebuild)
{
	const FilePath fullPath = FileSystem::FullPath(chartPath);
	if (!FileSystem::IsFile(fullPath) || !FsUtils::HasChartExtension(fullPath))
	{
		return none;
	}

	const String relativePath = FsUtils::RelativePathFromSongsDir(fullPath);
	...
	const FilePath packagePath = FileSystem::PathAppend(FsUtils::SongsDirectoryPath(), relativePath.substr(0, separatorPos));
	for (const auto& song : LoadDirectory(packagePath, forceRebuild))
	{
		for (int32 difficultyIdx = 0; difficultyIdx < kNumDifficulties; ++difficultyIdx)
		{
			const auto& chartInfo = song.chartInfos[difficultyIdx];
			if (chartInfo != nullptr && FileSystem::FullPath(chartInfo->chartFilePath()) == fullPath)
			{
				return SelectCachedChart{
					.chartFilePath = fullPath,
					.songDirectoryPath = song.songDirectoryPath,
...
Optional<SelectCachedSong> SelectMenuCacheDb::FindSong(FilePathView songOrChartPath, bool forceRebuild)
{
	const FilePath fullPath = FileSystem::FullPath(songOrChartPath);
	const String relativePath = FsUtils::RelativePathFromSongsDir(fullPath);
	if (relativePath.isEmpty() || relativePath.starts_with(U".."))
	{
		return none;
	...
	const FilePath packagePath = FileSystem::PathAppend(FsUtils::SongsDirectoryPath(), relativePath.substr(0, separatorPos));
	const FilePath targetSongDir = FileSystem::IsFile(fullPath) ? FileSystem::ParentPath(fullPath) : fullPath;
	const FilePath targetChartPath = FileSystem::IsFile(fullPath) ? fullPath : FilePath{};

	for (const auto& song : LoadDirectory(packagePath, forceRebuild))
	{
		if (FileSystem::FullPath(song.songDirectoryPath) == targetSongDir)
		{
			return song;
		}

		if (!targetChartPath.isEmpty())
		{
			for (const auto& chartInfo : song.chartInfos)
			{
				if (chartInfo != nullptr && FileSystem::FullPath(chartInfo->chartFilePath()) == targetChartPath)
				{
					return song;
				}
			}
