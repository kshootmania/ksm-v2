...
●Installation●
  Put "K-Shoot MANIA.app" into the "Applications" folder.

  If macOS shows "K-Shoot MANIA.app can't be opened because Apple cannot check it for malicious software" on first launch,
  press "Cancel", open "Privacy & Security" in System Settings, then choose "Open Anyway" → "Open" for K-Shoot MANIA.app.

  Songs and high scores are stored in "~/Library/Application Support/kshootmania".
  On first launch, a songs folder containing the default songs is created there.

●About●
  A music game where you operate keys to the music and play while applying effects to songs.
...
●Operation Guide● (During default settings)
  ...
  ◇Song Selection (With default key config)
    ...
    Hold Enter … Add/remove selected song from Favorites
    Shift+Enter … Autoplay selected song
      S,D,K,L   … Show various options (Change options using arrow keys)
     Command+F … Search songs in the folder
      D+K+英数  … Change player (To make a new player record, make a new folder in the "score" folder.)
       (or かな)
        Esc     … Close folder / Return to title screen

  ◇During Play (With default key config)
      S,D,K,L   … Hit BT (white) objects (H,J,F,G are also mapped to BT objects)
      英数,かな … Hit FX (orange) objects
       Space    … Treated as pressing both 英数 and かな keys
    ...
    ↑,↓,←,→ … Select song
      ...
      英数+かな … Show options
        Esc     … Return to title screen
                  (If pressed while songs are still downloading, download will be stopped. Only songs
                    that have finished downloading when Esc is pressed will be inputted.)
...
●License Notices for Libraries●
  See license.txt.


●Acknowledgements●
  BGM in this game was provided by RedMuffleR.
  Translation drafts (v1) were created by hakui for English, P.Lin* for Korean, and C.x.M for Chinese.
  The clap_punchy/clap_impact chip FX sound effects were provided by brz1128.
  Many others contributed songs, chart data, graphics, and other materials.
  We would like to express our sincere thanks to all contributors.
